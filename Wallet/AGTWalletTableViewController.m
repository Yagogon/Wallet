...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(addMoney)];
}
...
-(void) addMoney {
    ...
    NSUInteger randomMoney = arc4random_uniform(2);
    
        NSUInteger randomAmount = arc4random_uniform(500);
    
    if (randomMoney == EUROS) {
        [self.wallet plus:[AGTMoney euroWitAmount:randomAmount]];
    } else {
        [self.wallet plus:[AGTMoney dollarWithAmount:randomAmount]];
    }
